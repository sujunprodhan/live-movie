  ...
  return (
    <div className="bg-slate-900 min-h-screen py-12 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Back Button */}
        <Link to="/movies" className="inline-flex items-center text-emerald-400 hover:text-emerald-300 transition-colors mb-8">
        ...
        <div className="bg-slate-800 rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row border border-slate-700">
          ...
          <div className="md:w-1/3 shrink-0">
            <img 
              src={movie.image?.original || movie.image?.medium || 'https://via.placeholder.com/400x600?text=No+Image'} 
              alt={movie.name} 
          ...
          <div className="p-8 md:p-12 md:w-2/3 flex flex-col">
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              {movie.genres?.map(genre => (
                <span key={genre} className="px-3 py-1 text-sm font-semibold text-emerald-400 bg-emerald-400/10 rounded-full border border-emerald-400/20">
                  {genre}
                </span>
              ))}
            ...
            <div className="mt-auto grid grid-cols-2 sm:grid-cols-3 gap-6 bg-slate-900/50 p-6 rounded-xl border border-slate-700/50">
              <div>
                ...
                <p className="font-semibold">{movie.network?.name || movie.webChannel?.name || 'N/A'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-400 uppercase tracking-wider mb-1">Schedule</p>
                <p className="font-semibold">{movie.schedule?.days?.join(', ') || 'N/A'} at {movie.schedule?.time || ''}</p>
              </div>
              <div>
                <p className="text-sm text-gray-400 uppercase tracking-wider mb-1">Runtime</p>
                <p className="font-semibold">{movie.averageRuntime ? `${movie.averageRuntime} mins` : 'N/A'}</p>
              </div>
            </div>
